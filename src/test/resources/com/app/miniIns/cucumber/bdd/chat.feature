...
#    Message is sent to user2
    When  User sends message type "message" message "hello there" to "username2" through socket "websocket1"
    ...
    When User sends message type "ack" message "Received" to "server" through socket "websocket2"
...
#    Message is sent to user2
    ...
    When  User sends message type "message" message "hello there message 2" to "username2" through socket "websocket1"
    #From socket1 => ack message from handler； This is just to get the message, and store it before performing a series of checking
    Then consume message from websocket "websocket1"
    And message has "ack" for "$.type"
    And message has "has been Received on Server" for "$.message"
